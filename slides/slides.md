...
2. PRがmainにマージされると **GitHub Actionsが自動適用**
3. ルール変更はJSONを編集 → PRを出すだけ
...
## Tips：契約プログラミングでAI精度を上げる 📜

### `contract-check` ライブラリで関数の仕様を明示
...
- **事前条件**`pre()` / **事後条件**`post()` / **純粋性**`pure()` を宣言
- Copilotが関数の意図を正確に理解 → 生成コードの品質が向上
- ランタイムで契約違反を即座に検出
...
**1. GitHub Actions SHA pinning** — ActionをコミットSHAで固定
```yaml
